...
The basic idea is that the user writes a message to the AI agent, and it reads that, wipes the page, and "writes" out its reply. Currently it's a live back-and-forth chat rather than an asynchronous journal.
...
- **Android app**: captures handwritten input, sends it to a vision-capable LLM, and renders the streamed response back as handwriting, all on-device with no backend server involved.
- **LLM**: any OpenAI-compatible vision endpoint. You can run one locally ([LM Studio](https://lmstudio.ai/) and [Ollama](https://ollama.com/) are the easiest to set up; [llama.cpp](https://github.com/ggml-org/llama.cpp) and vLLM also work but need more manual setup) or use a hosted one like OpenRouter or Hugging Face Inference.
...
This confirms the APK was built by this repo's `release.yml` workflow from a specific commit. It was not tampered with or substituted after the fact.
...
A vision-capable LLM handles the recognition, and a local Caveat-font renderer handles the rendering. See [android-app/LOCAL_PROCESSING.md](android-app/LOCAL_PROCESSING.md) for how the pipeline works.
